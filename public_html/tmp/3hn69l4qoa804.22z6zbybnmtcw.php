    ...
    <div class="row">
	    <div class="col-12">
		    <?php if ($person): ?>  
	            ...
	            <h1><?= ($person['person_name'].'  '.$person['person_lname']) ?></h1>
	            <div class="row">
...
<form method="post" action="<?= ($BASE.'/project/listproject') ?>">   
    ...
    <div class="row">
	    ...
	    <?php if ($persontasks): ?>
                ...
                <tbody> 
    		    <?php $count=0; foreach (($persontasks?:[]) as $ptask): $count++; ?>
        	        <tr>
	               ...
	               <?php if ($ptask['type_entity'] === 'person'): ?>
                    ...
                    <?php endif; ?>             		    
            		    ...
            		    <td><?= ($getTaskStatusName($ptask['task_status'])) ?></td>           		    
                 	    <td>
...
<?php endif; ?>                 	        
   ...
   <!-- <?php if ($ptask['username'] == $SESSION['username']  || $SESSION['user_rol'] == 1): ?><?php endif; ?>     -->
        ...
        <a data-href="<?= ($BASE.'/task/tskdelete/'. $ptask['task_num'] . '/' . $entity_num . '/' . $ptask['type_entity'] . '/' . $myview) ?>" data-bs-toggle="modal" data-bs-target="#confirm-delete" title="<?= ($i18n_delete) ?>"><i style = "color:red;" class="bi bi-trash-fill" data-toggle="tooltip"></i></a>
...
<div class="modal fade" id="CreateTaskModal" tabindex="-1" aria-labelledby="TaskModalLabel" aria-hidden="true">
    ...
    <?php if ($SESSION['user_rol'] == 1): ?>
                ...
                <span class="input-group-text" id="myinputGroup-sizing-sm"><strong>Responsable:</strong></span>                     
        	     <select class="form-select bg-primary bg-opacity-10" id="username" name="username" required >
    ...
    <?php else: ?>
      		  ...
      		  <input type="text" class="form-control bg-primary bg-opacity-10" name="task_title" id="task_title"  value="<?= ($SESSION['username']) ?>" disabled>        
